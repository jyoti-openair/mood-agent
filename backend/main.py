...
FRONTEND_DIR = BASE_DIR / "frontend" if (BASE_DIR / "frontend").exists() else Path(__file__).resolve().parent / "frontend"
...
def download_pdfs_from_drive(folder_url: str) -> list[str]:
    """Helper to download entire Google Drive folders if folder URL is given."""
    os.makedirs(DATA_DIR, exist_ok=True)
    downloaded_files = gdown.download_folder(url=folder_url, output=str(DATA_DIR), quiet=False)
    if not downloaded_files:
        raise RuntimeError("Failed to download files from the provided Google Drive folder.")
    return [str(f) for f in downloaded_files if str(f).endswith(".pdf")]
...
async def reflect(req: ReflectRequest):
    ...
    try:
        ...
        # Format and clean response text for clear human readability
