document.addEventListener("DOMContentLoaded", () => {
  ...
  // Markdown parsing helper function
  function renderMarkdown(rawText) {
    if (!rawText) return '';
    
    // If marked.js is available, use it to convert Markdown to HTML
    if (typeof marked !== 'undefined') {
      return marked.parse(rawText);
    }
    
    // Fallback if marked library failed to load
    return rawText;
  }
  ...
  function displayResponse(mood, data) {
    ...
    // 1. Convert Markdown text response into HTML
    const textResponse = data.response || data.text || "";
    if (textResponse) {
      htmlContent += renderMarkdown(textResponse);
    }

    // 2. Embed Pollinations image visual if available
    const imageUrl = data.image_url || data.image_prompt_url;
    if (imageUrl) {
      htmlContent += `
        <div class="response-card__image-wrap" style="margin-top: 1.5rem;">
        <img 
          src="${imageUrl}" 
    ...
    // 3. Inject structured HTML into responseBody element
    if (responseBody) {
